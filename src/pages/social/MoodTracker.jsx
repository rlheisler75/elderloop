...
const MOODS = [
  { key: 'happy',     label: 'Happy',     emoji: '😊', color: 'bg-green-100 dark:bg-green-950/50 text-green-700 dark:text-green-400 border-green-200 dark:border-green-900'  },
  { key: 'content',   label: 'Content',   emoji: '😌', color: 'bg-teal-100 dark:bg-teal-950/50 text-teal-700 dark:text-teal-400 border-teal-200 dark:border-teal-900'    },
  { key: 'neutral',   label: 'Neutral',   emoji: '😐', color: 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-700' },
  { key: 'anxious',   label: 'Anxious',   emoji: '😟', color: 'bg-yellow-100 dark:bg-yellow-950/50 text-yellow-700 dark:text-yellow-400 border-yellow-200 dark:border-yellow-900' },
  { key: 'sad',       label: 'Sad',       emoji: '😢', color: 'bg-blue-100 dark:bg-blue-950/50 text-blue-700 dark:text-blue-400 border-blue-200 dark:border-blue-900'    },
  { key: 'agitated',  label: 'Agitated',  emoji: '😠', color: 'bg-orange-100 dark:bg-orange-950/50 text-orange-700 dark:text-orange-400 border-orange-200 dark:border-orange-900'},
  { key: 'confused',  label: 'Confused',  emoji: '😕', color: 'bg-purple-100 dark:bg-purple-950/50 text-purple-700 dark:text-purple-400 border-purple-200 dark:border-purple-900'},
  { key: 'withdrawn', label: 'Withdrawn', emoji: '😶', color: 'bg-indigo-100 dark:bg-indigo-950/50 text-indigo-700 dark:text-indigo-400 border-indigo-200 dark:border-indigo-900'},
  { key: 'combative', label: 'Combative', emoji: '😤', color: 'bg-red-100 dark:bg-red-950/50 text-red-700 dark:text-red-400 border-red-200 dark:border-red-900'       },
  { key: 'other',     label: 'Other',     emoji: '🔵', color: 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-700' },
]
...
function QuickLogModal({ residents, orgId, onClose, onSaved }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 p-0 sm:p-4">
      <div className="bg-white dark:bg-slate-900 w-full sm:max-w-lg rounded-t-3xl sm:rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex-shrink-0">
          <div>
            <h2 className="font-display font-bold text-slate-800 dark:text-slate-100">Quick Mood Log</h2>
            <div className="flex gap-2 mt-1">
              {[1,2,3].map(s => (
                <div key={s} className={`h-1.5 w-8 rounded-full transition-colors ${step >= s ? 'bg-brand-600' : 'bg-slate-200 dark:bg-slate-700'}`} />
              ))}
            </div>
          </div>
            ...
            <div className="p-5 space-y-3">
              ...
              <div className="relative">
                ...
                <input value={search} onChange={e => setSearch(e.target.value)} autoFocus
                  ...
                  className="w-full pl-9 pr-3 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 dark:bg-slate-800 dark:text-slate-100" />
              </div>
              <div className="space-y-1 max-h-64 overflow-y-auto">
                {filtered.map(r => (
                  <button key={r.id} onClick={() => { set('resident_id', r.id); setStep(2); setSearch('') }}
                    className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-brand-50 dark:hover:bg-slate-800 transition-colors text-left group">
                    ...
                    <div>
                      <div className="font-medium text-slate-800 dark:text-slate-100 text-sm">{r.first_name} {r.last_name}</div>
          ...
          {step === 2 && (
            <div className="p-5 space-y-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="font-semibold text-slate-800 dark:text-slate-100">{selectedResident?.first_name} {selectedResident?.last_name}</div>
                  <div className="text-xs text-slate-400">Room {selectedResident?.room}</div>
                </div>
              </div>

              <div>
                <p className="text-sm font-medium text-slate-600 dark:text-slate-300 mb-3">How is {selectedResident?.first_name} feeling?</p>
                  ...
                  {MOODS.map(m => (
                    <button key={m.key} onClick={() => { set('mood', m.key); setStep(3) }}
                      className={`flex items-center gap-2.5 px-4 py-3 rounded-xl border-2 font-medium text-sm transition-all
                        ${form.mood === m.key ? m.color + ' border-current' : 'bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:border-slate-300'}`}>
              ...
              <div>
                <p className="text-sm font-medium text-slate-600 dark:text-slate-300 mb-2">Shift</p>
                <div className="flex gap-2">
                  {SHIFTS.map(s => (
                    <button key={s.key} onClick={() => set('shift', s.key)}
                      className={`flex-1 py-2 rounded-xl border text-sm font-medium transition-all
                        ${form.shift === s.key ? 'bg-brand-600 text-white border-brand-600' : 'bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-300 border-slate-200 dark:border-slate-700 hover:border-brand-300'}`}>
                      {s.label}
                    </button>
                  ))}
                </div>
