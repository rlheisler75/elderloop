  ...
  async function fetchAll() {
    ...
    if (hasModule('activities')) {
      ...
      queries.push(
        ...
        supabase.from('activity_attendance').select('resident_id,occurrence_date').eq('organization_id', orgId).gte('occurrence_date', `${attendanceCutoff.getFullYear()}-${String(attendanceCutoff.getMonth()+1).padStart(2,'0')}-${String(attendanceCutoff.getDate()).padStart(2,'0')}`)
      )
