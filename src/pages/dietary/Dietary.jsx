            ...
            <div className="grid grid-cols-3 gap-2">
              {DIET_TYPES.map(d => (
                <button key={d.key} onClick={() => set('diet_type', d.key)}
                  className={`px-3 py-2 rounded-lg border text-xs font-medium transition-all text-left ${form.diet_type === d.key ? 'bg-brand-600 text-white border-brand-600' : 'border-slate-200 text-slate-600 hover:border-brand-300'}`}>
                  {d.label}
                </button>
              ))}
            </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {CONSISTENCIES.map(c => (
                <button key={c.key} onClick={() => set('consistency', c.key)}
                  className={`px-3 py-2 rounded-lg border text-xs font-medium transition-all text-left ${form.consistency === c.key ? 'bg-brand-600 text-white border-brand-600' : 'border-slate-200 text-slate-600 hover:border-brand-300'}`}>
                  {c.label}
                </button>
              ))}
            </div>
