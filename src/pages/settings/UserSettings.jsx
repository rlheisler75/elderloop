...
import { Palette, Check, Mail, Bell, Smartphone, Loader2, AlertCircle } from 'lucide-react'
...
function Toggle({ checked, onChange, disabled }) {
  return (
    <button onClick={() => !disabled && onChange(!checked)} disabled={disabled}
      className={`relative w-11 h-6 rounded-full transition-colors flex-shrink-0 ${checked ? 'bg-brand-600' : 'bg-slate-200'} ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}>
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-slate-500 text-sm mt-0.5">Personal preferences for your account</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 mb-5">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h2 className="font-display font-semibold text-slate-800">Accent Color</h2>
        </div>
        ...
        <div className="grid grid-cols-5 gap-3">
          {ACCENT_COLORS.map(c => (
            <button key={c.key} onClick={() => handleAccent(c.key)}
              className={`flex flex-col items-center gap-2 p-3 rounded-xl border-2 transition-all ${accent === c.key ? 'border-brand-500 bg-brand-50' : 'border-slate-200 hover:border-slate-300'}`}>
              ...
              <span className="text-xs font-medium text-slate-600 text-center leading-tight">{c.label}</span>
            </button>
          ))}
        </div>
