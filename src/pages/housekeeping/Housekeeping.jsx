...
// ── Status helpers ─────────────────────────────────────────────
const IL_STATUSES = [
  { key: 'pending',     label: 'Pending',     color: 'bg-yellow-50 text-yellow-700 border-yellow-200 dark:bg-yellow-950/50 dark:text-yellow-400 dark:border-yellow-900', dot: 'bg-yellow-400' },
  { key: 'booked',      label: 'Booked',      color: 'bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-950/50 dark:text-blue-400 dark:border-blue-900',       dot: 'bg-blue-500' },
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800">
          <h2 className="font-display font-semibold text-slate-800 dark:text-slate-100">Cleaning Receipt</h2>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div>
            ...
            <input value={form.phone} onChange={e => set('phone', e.target.value)}
              className="w-full mt-2 px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-100"
            ...
            <div className="flex flex-wrap gap-2">
              {IL_STATUSES.map(s => (
                <button key={s.key} onClick={() => set('status', s.key)}
                  className={`px-3 py-1.5 rounded-lg border text-xs font-medium transition-all ${form.status === s.key ? s.color + ' ring-2 ring-offset-1 ring-brand-400' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:border-slate-300 dark:hover:border-slate-600'}`}>
                  {s.label}
                </button>
              ))}
            </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                <div className="col-span-1">
                  ...
                  <input type="date" value={form.booked_date} onChange={e => set('booked_date', e.target.value)}
                    className="w-full px-3 py-2 border border-blue-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 bg-white dark:bg-slate-800 dark:border-blue-900 dark:text-slate-100" />
                </div>
                <div>
                ...
                <div>
                  <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">Est. Hours</label>
          ...
          {/* Completion / billing */}
          {showCompletion && (
            <div className="p-4 bg-green-50 border border-green-100 rounded-xl space-y-3 dark:bg-green-950/30 dark:border-green-900">
              ...
              <div>
                <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">Actual Hours</label>
                <input type="number" step="0.5" min="0.5" value={form.actual_hours} onChange={e => set('actual_hours', e.target.value)}
                  className="w-full px-3 py-2 border border-green-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-green-400 bg-white dark:bg-slate-800 dark:border-green-900 dark:text-slate-100"
                  placeholder="Actual hours worked" />
              </div>
              <div>
                <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">Services Performed</label>
              ...
              <div>
                <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">Completion Notes</label>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-lg max-h-[92vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex-shrink-0">
          <div>
            <h2 className="font-display font-semibold text-slate-800 dark:text-slate-100">Inspection — {area.name}</h2>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-3">
          {checklistItems.map(item => (
            <div key={item.id} className={`p-3 rounded-xl border transition-all ${results[item.id]?.passed ? 'bg-green-50 border-green-200 dark:bg-green-950/30 dark:border-green-900' : 'bg-red-50 border-red-200 dark:bg-red-950/30 dark:border-red-900'}`}>
              ...
              {!results[item.id]?.passed && (
                <input value={results[item.id]?.note || ''} onChange={e => setNote(item.id, e.target.value)}
                  className="w-full mt-2 px-3 py-1.5 border border-red-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-red-400 bg-white dark:bg-slate-800 dark:border-red-900 dark:text-slate-100"
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-display text-2xl font-semibold text-slate-800 dark:text-slate-100">Housekeeping</h1>
          ...
          {showAddArea && (
            <div className="mb-4 p-4 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl flex items-center gap-3">
              ...
              <select value={newAreaType} onChange={e => setNewAreaType(e.target.value)}
                className="px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 dark:bg-slate-900 dark:border-slate-700 dark:text-slate-100">
              ...
              <button onClick={() => setShowAddArea(false)} className="text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300"><X size={18} /></button>
            </div>
          )}
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-8">
                {areas.map(area => {
                  ...
                  return (
                    <div key={area.id} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm p-4 hover:shadow-md transition-all">
        ...
        <div>
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {filteredIL.map(req => (
                <div key={req.id} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm p-5 hover:shadow-md transition-all">
