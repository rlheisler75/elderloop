...
const ROLE_LABELS = {
  ...
  resident:    { label: 'Resident',    color: 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400' },
  family:      { label: 'Family',      color: 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400' },
  super_admin: { label: 'Super Admin', color: 'bg-purple-100 dark:bg-purple-950/50 text-purple-800 dark:text-purple-400' },
}

function RoleBadge({ role }) {
  const r = ROLE_LABELS[role] || { label: role, color: 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300' }
      ...
      <button onClick={() => setExpanded(e => !e)}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="font-medium text-slate-800 dark:text-slate-100 text-sm">{user.first_name} {user.last_name}</span>
                  ...
                  return (
                    <button key={moduleKey} onClick={() => handleToggle(moduleKey, perm)}
                      ...
                      className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-xs font-medium transition-all text-left ${
                        ...
                        level === 'view' ? 'bg-amber-50 dark:bg-amber-950/50 border-amber-300 dark:border-amber-800 text-amber-700 dark:text-amber-400' :
...
export default function UserPermissions({ orgId, orgModules }) {
  ...
  return (
    <div>
      ...
      {/* Legend */}
      <div className="flex items-center gap-4 mb-4 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-800 text-xs">
        <span className="text-slate-500 dark:text-slate-400 font-medium">Access levels:</span>
        ...
        <div className="flex items-center gap-1.5 text-slate-400">
          <div className="w-5 h-5 rounded bg-slate-200 dark:bg-slate-700 flex items-center justify-center"><X size={9} className="text-slate-400" /></div>
          No access
        </div>
      </div>
