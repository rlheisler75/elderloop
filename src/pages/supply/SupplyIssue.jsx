...
export default function SupplyIssue() {
  ...
  return (
    <div className="p-6 max-w-4xl">
      <h2 className="font-display font-semibold text-slate-800 text-lg mb-1">Issue / Checkout</h2>
      ...
      <div className="flex gap-2 mb-6">
        {[{ k: 'dept', label: 'Department', icon: Building2 }, { k: 'resident', label: 'Resident', icon: User }].map(m => (
          <button key={m.k} onClick={() => { setIssueType(m.k); setLines([]) }}
            className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border text-sm font-medium transition-all ${issueType === m.k ? 'bg-brand-600 text-white border-brand-600' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
          ...
          <div className="flex gap-2 flex-wrap">
            {DEPARTMENTS.map(d => (
              <button key={d} onClick={() => setSelectedDept(d)}
                className={`px-3 py-1.5 rounded-xl border text-sm font-medium transition-all ${selectedDept === d ? 'bg-brand-600 text-white border-brand-600' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                {d}
              </button>
            ))}
          </div>
          ...
          {selectedResident ? (
            <div className="flex items-center gap-3 p-3 bg-brand-50 border border-brand-200 rounded-xl">
              ...
              <div className="flex-1">
                <div className="font-medium text-slate-800">{selectedResident.first_name} {selectedResident.last_name}</div>
                {selectedResident.room && <div className="text-xs text-slate-500">Room {selectedResident.room}</div>}
              </div>
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <input value={resSearch} onChange={e => setResSearch(e.target.value)} placeholder="Search resident by name or room..." className="w-full pl-9 pr-4 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 bg-white" />
              </div>
              <button onClick={() => setScanMode('resident')} className="flex items-center gap-2 px-4 py-2 border border-slate-200 text-slate-600 hover:bg-slate-50 rounded-xl text-sm font-medium transition-colors">
                <QrCode size={15} /> Scan ID
              </button>
            </div>
          )}
          {resSearch && !selectedResident && (
            <div className="mt-2 bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden max-h-40 overflow-y-auto">
              {filteredResidents.slice(0, 6).map(r => (
                <button key={r.id} onClick={() => { setSelectedResident(r); setResSearch('') }}
                  ...
                  <div className="text-sm font-medium text-slate-800">{r.first_name} {r.last_name}</div>
      ...
      <div className="mb-4 flex gap-2">
        ...
        <button onClick={() => setScanMode('item')} className="flex items-center gap-2 px-4 py-2 border border-slate-200 text-slate-600 hover:bg-slate-50 rounded-xl text-sm font-medium transition-colors">
      ...
      {lines.length > 0 && (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden mb-5">
          <div className="px-5 py-3 border-b border-slate-100 bg-slate-50">
            <span className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Items to Issue</span>
          </div>
      ...
      {lines.length > 0 && (
        <>
          <div className="mb-4">
            ...
            <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={2} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 resize-none" placeholder="Optional notes..." />
          </div>
