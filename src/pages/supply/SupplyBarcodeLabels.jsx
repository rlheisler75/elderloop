// SupplyBarcodeLabels.jsx
// Requires: npm install qrcode --legacy-peer-deps
import { useState, useEffect, useRef } from 'react'
import { supabase } from '../../lib/supabase'
import { useAuth } from '../../context/AuthContext'
import QRCode from 'qrcode'
import { X, Printer, Package, Search, CheckSquare, Square } from 'lucide-react'
...
function Label({ item, qrDataUrl, size }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0, overflow: 'hidden' }}>
        ...
        {item.category && (
          <div style={{ fontSize: s.fontSize, color: '#555', marginBottom: 1 }}>{item.category}</div>
        )}
        {item.sku && (
          <div style={{ fontSize: s.fontSize, color: '#444', fontFamily: 'monospace' }}>SKU: {item.sku}</div>
        )}
        {item.barcode && (
...
export default function SupplyBarcodeLabels({ onClose }) {
  ...
  async function fetchItems() {
    const { data } = await supabase.from('supply_items')
      .select('id, name, category, barcode, sku, location')
  ...
  const filtered = items.filter(i => {
    ...
    return i.name.toLowerCase().includes(q) || i.category?.toLowerCase().includes(q) || i.sku?.toLowerCase().includes(q) || i.location?.toLowerCase().includes(q)
  })
  ...
  const handlePrint = () => {
      ...
      return `
        <div style="width:${s.w};height:${s.h};border:1px solid #ccc;border-radius:4px;display:flex;align-items:center;gap:8px;padding:6px;box-sizing:border-box;page-break-inside:avoid;break-inside:avoid;background:#fff;">
          ...
          <div style="flex:1;min-width:0;overflow:hidden;font-family:sans-serif;">
            ...
            ${item.category ? `<div style="font-size:${s.fontSize}px;color:#555;margin-bottom:1px;">${item.category}</div>` : ''}
            ${item.sku ? `<div style="font-size:${s.fontSize}px;color:#444;font-family:monospace;">SKU: ${item.sku}</div>` : ''}
            ${item.barcode ? `<div style="font-size:${s.fontSize}px;color:#444;font-family:monospace;">${item.barcode}</div>` : ''}
    ...
    printWindow.document.write(`
      <!DOCTYPE html><html><head><title>Supply Labels</title>
      <style>
            ...
            <div className="flex-1 overflow-auto p-5 bg-slate-50 dark:bg-slate-800">
              {generating ? (
                ...
                <div className="flex flex-col items-center justify-center h-32 text-slate-400">
                  ...
                  <p className="text-sm">Select items to preview labels</p>
                </div>
              ) : (
                <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(s.cols, 2)}, auto)`, gap: '8px' }}>
                  {printItems.slice(0, 12).map((item, i) => (
                    <Label key={`${item.id}-${i}`} item={item} qrDataUrl={qrMap[item.id]} size={size} />
                  ))}
