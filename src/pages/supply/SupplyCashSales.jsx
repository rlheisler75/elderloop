...
export default function SupplyCashSales() {
  ...
  return (
    <div className="p-6 max-w-4xl">
      <h2 className="font-display font-semibold text-slate-800 dark:text-slate-100 text-lg mb-1">Cash Sales</h2>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-sm">
            <div className="px-6 py-5 border-b border-slate-100 dark:border-slate-800 text-center">
              ...
              <h2 className="font-display font-semibold text-slate-800 dark:text-slate-100 text-lg">Sale Complete</h2>
              <p className="text-xs text-slate-400 mt-0.5">{receipt.date}</p>
            </div>
            <div className="px-6 py-4 space-y-2">
              <div className="flex justify-between text-sm"><span className="text-slate-500 dark:text-slate-400">Buyer</span><span className="font-medium text-slate-800 dark:text-slate-100">{receipt.buyer}</span></div>
              <div className="border-t border-slate-100 dark:border-slate-800 pt-2 mt-2 space-y-1">
                {receipt.lines.map((l, i) => (
                  <div key={i} className="flex justify-between text-sm">
                    <span className="text-slate-600 dark:text-slate-300">{l.description} × {l.quantity}</span>
                    <span className="text-slate-800 dark:text-slate-100">${(Number(l.quantity) * Number(l.sale_price)).toFixed(2)}</span>
                  </div>
                ))}
              </div>
              <div className="border-t border-slate-100 dark:border-slate-800 pt-2 space-y-1">
                <div className="flex justify-between text-sm font-semibold text-slate-800 dark:text-slate-100"><span>Total</span><span>${receipt.total.toFixed(2)}</span></div>
                <div className="flex justify-between text-sm"><span className="text-slate-500 dark:text-slate-400">Tendered</span><span className="text-slate-800 dark:text-slate-100">${receipt.tendered.toFixed(2)}</span></div>
            ...
            <div className="px-6 pb-5 flex gap-2">
              <button onClick={() => window.print()} className="flex-1 flex items-center justify-center gap-2 py-2.5 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 rounded-xl text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-800"><Printer size={14} /> Print</button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div>
          {/* Staff buyer */}
          <div className="mb-4">
            <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-2">Buyer (Staff)</label>
            {selectedStaff ? (
              <div className="flex items-center gap-3 p-3 bg-brand-50 dark:bg-brand-950/30 border border-brand-200 dark:border-brand-800 rounded-xl">
                ...
                <div className="flex-1">
                  <div className="font-medium text-slate-800 dark:text-slate-100 text-sm">{selectedStaff.first_name} {selectedStaff.last_name}</div>
              ...
              <>
                <div className="relative">
                  ...
                  <input value={staffSearch} onChange={e => setStaffSearch(e.target.value)} placeholder="Search staff member..." className="w-full pl-9 pr-4 py-2 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 bg-white dark:bg-slate-800 dark:text-slate-100" />
                </div>
                {staffSearch && (
                  <div className="mt-1 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden max-h-36 overflow-y-auto">
                    {filteredStaff.slice(0, 6).map(s => (
                      <button key={s.id} onClick={() => { setSelectedStaff(s); setStaffSearch('') }} className="w-full flex items-center justify-between px-4 py-2.5 hover:bg-slate-50 dark:hover:bg-slate-800 border-b border-slate-100 dark:border-slate-800 last:border-0 text-left">
                        <span className="text-sm font-medium text-slate-800 dark:text-slate-100">{s.first_name} {s.last_name}</span>
          ...
          <div className="mb-4 flex gap-2">
            <div className="relative flex-1">
              ...
              <input value={itemSearch} onChange={e => setItemSearch(e.target.value)} placeholder="Search items for sale..." className="w-full pl-9 pr-4 py-2 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 bg-white dark:bg-slate-800 dark:text-slate-100" />
            </div>
            ...
            <div className="mb-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden max-h-48 overflow-y-auto">
              {filteredItems.slice(0, 8).map(item => (
                <button key={item.id} onClick={() => addLine(item)}
                  ...
                  <span className="text-sm font-semibold text-green-700 dark:text-green-400">${Number(item.sale_price).toFixed(2)}</span>
                </button>
              ))}
          ...
          {items.length === 0 && !loading && (
            <div className="p-4 bg-amber-50 dark:bg-amber-950/50 border border-amber-200 dark:border-amber-900 rounded-xl text-sm text-amber-700 dark:text-amber-400">
        ...
        <div>
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden mb-4">
            <div className="px-5 py-3 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800 flex items-center justify-between">
              <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Cart</span>
              ...
              <div className="divide-y divide-slate-100 dark:divide-slate-800">
                {lines.map((line, idx) => (
                  <div key={idx} className="flex items-center gap-3 px-4 py-3">
                    ...
                    <input type="number" min="1" value={line.quantity} onChange={e => setLineQty(idx, e.target.value)} className="w-14 px-2 py-1 border border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 rounded-lg text-sm text-center focus:outline-none focus:ring-2 focus:ring-brand-500" />
                    <span className="text-xs text-slate-400">×</span>
                    <div className="relative">
                      <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400 text-xs">$</span>
                      <input type="number" min="0" step="0.01" value={line.sale_price} onChange={e => setLinePrice(idx, e.target.value)} className="w-20 pl-5 pr-2 py-1 border border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-500" />
                    </div>
                    <span className="text-sm font-semibold text-slate-800 dark:text-slate-100 w-16 text-right">${(Number(line.quantity) * Number(line.sale_price || 0)).toFixed(2)}</span>
                ...
                <div className="px-4 py-3 bg-slate-50 dark:bg-slate-800 flex items-center justify-between">
                  ...
                  <span className="font-display font-bold text-slate-800 dark:text-slate-100 text-lg">${total.toFixed(2)}</span>
                </div>
              </div>
            )}
          </div>
          ...
          {lines.length > 0 && (
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1.5">Amount Tendered ($)</label>
              ...
              <button onClick={handleSubmit} disabled={!canSubmit || saving}
                className="w-full flex items-center justify-center gap-2 py-3 bg-green-600 hover:bg-green-700 disabled:bg-slate-200 dark:disabled:bg-slate-700 disabled:text-slate-400 dark:disabled:text-slate-500 text-white font-medium rounded-xl shadow-sm transition-colors">
                <DollarSign size={16} />
