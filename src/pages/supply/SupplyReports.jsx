...
const TYPE_CFG = {
  receive:       { label: 'Received',       color: 'bg-green-100 text-green-700',   icon: ArrowUpCircle },
  issue_dept:    { label: 'Dept Issue',     color: 'bg-blue-100 text-blue-700',     icon: ArrowDownCircle },
  issue_resident:{ label: 'Resident Charge',color: 'bg-purple-100 text-purple-700', icon: User },
  cash_sale:     { label: 'Cash Sale',      color: 'bg-amber-100 text-amber-700',   icon: DollarSign },
  adjustment:    { label: 'Adjustment',     color: 'bg-slate-100 text-slate-600',   icon: RefreshCw },
}
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        <div>
          <h2 className="font-display font-semibold text-slate-800 text-lg">Reports</h2>
        ...
        <div className="flex items-center gap-2">
          ...
          {[7, 30, 90].map(d => (
            <button key={d} onClick={() => setDays(d)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${days === d ? 'bg-brand-600 text-white border-brand-600' : 'border-slate-200 text-slate-500 hover:bg-slate-50'}`}>
              {d}d
            </button>
          ))}
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {[
          { label: 'Inventory Value',  value: `$${totalValue.toFixed(2)}`, sub: 'at cost',             color: 'text-slate-800', icon: Package },
      ...
      <div className="flex gap-0 border-b border-slate-100 mb-5 overflow-x-auto">
        {REPORT_TABS.map(t => (
          <button key={t.k} onClick={() => setReportTab(t.k)}
            className={`px-4 py-2.5 text-sm font-medium border-b-2 whitespace-nowrap transition-all ${reportTab === t.k ? 'border-brand-600 text-brand-700' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {reportTab === 'overview' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
            <h3 className="font-display font-semibold text-slate-700 mb-3">Usage by Category</h3>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
            <h3 className="font-display font-semibold text-slate-700 mb-3">Usage by Department</h3>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          {lowItems.length === 0 ? (
            <div className="text-center py-12 text-slate-400">
              ...
              <p className="font-display text-slate-600">All items are at or above par level</p>
            </div>
          ) : (
                ...
                {lowItems.map(item => {
                  ...
                  return (
                    <tr key={item.id} className="border-b border-slate-100 last:border-0">
                      ...
                      <td className="px-4 py-3"><span className="text-xs px-2 py-0.5 rounded-full bg-slate-100 text-slate-600">{item.category}</span></td>
                      ...
                      <td className="px-4 py-3 text-sm text-slate-500">{item.par_level} {item.unit}</td>
                      <td className="px-4 py-3 text-sm text-slate-500">{item.reorder_point} {item.unit}</td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium ${status === 'out' ? 'bg-slate-100 text-slate-500' : status === 'critical' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>
      ...
      {reportTab === 'history' && (
        <>
          <div className="flex gap-2 flex-wrap mb-4">
            <button onClick={() => setTxType('all')} className={`px-3 py-1.5 rounded-xl text-xs font-medium border transition-all ${txType === 'all' ? 'bg-brand-600 text-white border-brand-600' : 'border-slate-200 text-slate-500 hover:bg-slate-50'}`}>All</button>
            {Object.entries(TYPE_CFG).map(([k, v]) => (
              <button key={k} onClick={() => setTxType(k)} className={`px-3 py-1.5 rounded-xl text-xs font-medium border transition-all ${txType === k ? 'bg-brand-600 text-white border-brand-600' : 'border-slate-200 text-slate-500 hover:bg-slate-50'}`}>{v.label}</button>
            ))}
          </div>
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            {loading ? (
                  ...
                  {filteredTx.slice(0, 100).map(tx => {
                    ...
                    return (
                      <tr key={tx.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50">
                        ...
                        <td className="px-4 py-2.5 text-sm text-slate-700">{tx.supply_items?.name || '—'}</td>
      ...
      {reportTab === 'usage' && (
        <div className="space-y-4">
          {Object.entries(deptUsage).length === 0 ? (
            ...
            return (
              <div key={dept} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-display font-semibold text-slate-800">{dept}</h3>
