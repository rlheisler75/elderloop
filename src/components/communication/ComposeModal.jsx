...
import { X, Mail, MessageSquare, Bell, ChevronDown, ChevronUp, Send, Loader2,
         Users, User, Building2, Heart, Baby, Stethoscope, Wrench, UtensilsCrossed,
...
const SYSTEM_TEMPLATES = [
  ...
  { name: 'Medication Reminder', subject: 'Medication reminder', body: 'This is a reminder to take your scheduled medication. Please contact nursing if you have any questions.', category: 'health' },
  { name: 'Visitor Policy Update', subject: 'Visitor policy update', body: 'We have an update regarding our visitor policies. Please review the following information carefully.', category: 'general' },
  { name: 'Maintenance Notice', subject: 'Scheduled maintenance notice', body: 'Please be aware of scheduled maintenance in your area. We apologize for any inconvenience and will complete the work as quickly as possible.', category: 'reminder' },
]
...
export default function ComposeModal({ onClose, onSent, prefill = null }) {
  ...
  useEffect(() => {
    ...
    // Load people for individual targeting
    supabase.from('profiles').select('id, full_name, role, email, phone')
  ...
  const filteredPeople = () => {
    ...
    return allPeople.filter(p => p.full_name?.toLowerCase().includes(s) || p.email?.toLowerCase().includes(s))
  }
  ...
  async function handleSend() {
    ...
    try {
      // 1. Insert the broadcast message
      ...
      // 2. Call edge function to actually deliver
      const { error: fnErr } = await supabase.functions.invoke('send-broadcast', {
        body: { message_id: msg.id }
      })

      if (fnErr) {
        console.warn('Edge function error (non-fatal):', fnErr)
        // Even if edge function fails, we still created the record
      }
  ...
  const audienceOptions = [
    ...
    { key: 'all_residents', label: 'All Residents',   icon: Baby,      desc: 'All current residents' },
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
          ...
          <div>
            ...
            <div className="flex gap-2">
              {[
                { key: 'push',  label: 'In-App',  icon: Bell,          color: 'brand' },
            ...
            {smsWarning && form.channels.includes('sms') && (
              <p className="text-xs text-yellow-700 bg-yellow-50 border border-yellow-200 rounded-lg px-3 py-2 mt-2">
                ⚠️ SMS delivery requires Twilio A2P 10DLC carrier registration (~2-4 weeks). Messages will be queued and delivered once registration is complete.
              </p>
            )}
          </div>
                ...
                return (
                  <button key={cat.key} onClick={() => set('category', cat.key)}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-medium transition-all
                      ${form.category === cat.key ? cat.color + ' ring-2 ring-offset-1 ring-brand-300' : 'border-slate-200 text-slate-500 hover:border-slate-300 bg-white'}`}>
                    <Icon size={13} />
                    {cat.label}
                  </button>
                )
            ...
            {form.audience_type === 'department' && (
              <div className="mt-3 grid grid-cols-2 gap-2">
                {DEPARTMENTS.map(d => {
                  const Icon = d.icon
                  return (
                    <button key={d.key} onClick={() => set('audience_dept', d.key)}
                      className={`flex items-center gap-2 px-3 py-2.5 rounded-xl border-2 text-sm transition-all
                        ${form.audience_dept === d.key ? 'bg-brand-50 border-brand-400 text-brand-700 font-medium' : 'bg-white border-slate-200 text-slate-600 hover:border-slate-300'}`}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-slate-100 flex-shrink-0">
          <div className="text-xs text-slate-400">
            ...
            {' → '}
            {form.audience_type === 'all' ? 'Everyone'
              : form.audience_type === 'all_staff' ? 'All Staff'
              : form.audience_type === 'all_residents' ? 'All Residents'
              : form.audience_type === 'all_family' ? 'All Family'
              : form.audience_type === 'department' ? (form.audience_dept || 'Select dept...')
