...
export default function PushPermissionModal({ profileId, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
      <div className={`bg-white rounded-2xl shadow-2xl w-full max-w-sm transition-all duration-300 ${declined ? 'opacity-0 translate-y-4' : 'opacity-100'}`}>
        {/* Header */}
        <div className="flex items-start justify-between p-5 pb-0">
          ...
          <button onClick={handleDecline} className="p-1.5 text-slate-300 hover:text-slate-500 transition-colors">
            <X size={16} />
          </button>
        </div>
        ...
        <div className="px-5 py-4">
          <h3 className="font-display font-bold text-slate-800 text-lg mb-1.5">
            Stay notified
          </h3>
          ...
          <div className="flex items-center gap-2 text-xs text-slate-400 mb-5 bg-slate-50 rounded-xl px-3 py-2">
        ...
        <div className="px-5 pb-5 flex flex-col gap-2">
          ...
          <button
            ...
            className="w-full py-2.5 text-slate-400 hover:text-slate-600 text-sm font-medium transition-colors flex items-center justify-center gap-1.5">
